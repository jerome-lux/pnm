...
parser.add_option("-f", "--file", dest="filename", default=None,
                  help="FILE to process. If not present all files with file with .network extension are processed", metavar="FILE")
parser.add_option("-l", "--label", dest="label", action="store_true",
                  help="Render labels instead of saturation")
parser.add_option("-p", "--pressure", dest="pressure",
                  action="store_true", help="Render pressure instead of saturation")
...
parser.add_option("--xperiodic", dest="xperiodic", action="store_true",
                  help="Network is periodic in the x direction")
parser.add_option("--yperiodic", dest="yperiodic", action="store_true",
                  help="Network is periodic in the y direction")
parser.add_option("--zperiodic", dest="zperiodic", action="store_true",
                  help="Network is periodic in the z direction")
...
def Net_draw(im, res):
    ...
    if options.label:
        maxval = max(max(int(caps[i][4]) for i in range(len(caps))), max(
            int(pores[j][7]) for j in range(len(pores))))
        print ("label max=", maxval)
        label = [[0, i] for i in range(int(maxval))]
        for i in range(ncaps):
            if int(caps[i][4]) > 0:
                label[int(caps[i][4])-1][0] += 1
        for i in range(npores):
            if int(pores[i][7]) > 0:
                label[int(pores[i][7])-1][0] += 1
        ...
        if options.prange == (0, 0):
            maxp = max(pressure(j) for j in range(npores))
            minp = min(pressure(j) for j in range(npores))
            if minp == maxp:
                minp = 0
                maxp = 1
            print(minp, maxp)
    ...
    for i in range(ncaps):
        ...
        if (options.xperiodic and (CL & 768) == 768) or (options.yperiodic and (CL & 3072) == 3072) or (options.zperiodic and (CL & 12288) == 12288):
            ...
            if not (options.label or options.pressure):
                r, g, b = int(
                    (1.0-float(caps[i][3]))*255), int((1.0-float(caps[i][3]))*255), 255
            elif(options.label):
                if int(caps[i][4]) > 0:
                    r, g, b = colorsys.hsv_to_rgb(float(label[int(caps[i][4])-1][0])/float(
                        maxval), float(int(label[int(caps[i][4])-1][0]) % 2)/2+0.5, 1)
                    r, g, b = int(r*255), int(g*255), int(b*255)
                else:
                    r, g, b = 255, 255, 255
            elif(options.pressure):
                r, g, b = colorsys.hsv_to_rgb(
                    0.664*(normp(int(caps[i][0])-1) + normp(int(caps[i][1])-1))/2, 1, 1)
                r, g, b = int(r*255), int(g*255), int(b*255)
            draw.rectangle((cx-rad, cy-rad, cx2+rad, cy2+rad),
    ...
    for i in range(npores):
        ...
        if not (options.label or options.pressure):
            r, g, b = int(
                (1.0-float(pores[i][4]))*255), int((1.0-float(pores[i][4]))*255), 255
            if options.points:
            ...
            if int(pores[i][7]) > 0:
                r, g, b = colorsys.hsv_to_rgb(float(label[int(pores[i][7])-1][0])/float(
                    maxval), float(int(label[int(pores[i][7])-1][0]) % 2)/2+0.5, 1)
                r, g, b = int(r*255), int(g*255), int(b*255)
...
if options.filename != None:
    ...
    for filename in glob.glob('*.network'):
        ...
        counter = counter+1
        npores, ncaps, header, caps, pores = network_reader(
            header, caps, pores)
        t_size, res = GetRes(t_size)
        image = Image.new("RGB", t_size, "white")
        Net_draw(image, res)
