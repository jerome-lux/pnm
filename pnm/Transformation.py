...
def assemble_pnm(pn1,pn2,axis='x',ratio=1,minc=3,maxc=np.inf,BCgridsize=100,TRR=1):
    ...
    if pn1.graph.graph['graph_type'] == 'cubic_regular' or pn2.graph.graph['graph_type'] == 'cubic_regular':
        ...
        add_throats_by_surf(newpnm,nodes=nodes_to_connect1,ratio=ratio,minc=minc,maxc=maxc,sort_by_radius=False,TRR=TRR)
...
def add_throats(net,lowc=0,highc=np.inf,minc=0,maxc=np.inf,mode='auto',sort_by_radius=True, spacing = 'auto'):
    ...
    if spacing == 'auto':
        try:
            ...
            spacing = pores_radii.sum() / pores_radii.size  #Moyenne arithmétique des rayons de pore
    ...
    candidates.sort(key = lambda t:t[1],reverse=True)       #On traite les plus gros pores en premier
    ...
    for n1 in candidates:
        ...
        indices, distances = kdtree.query_radius([net.graph.nodes[n1]['center']], (max_pore_radius + net.graph.nodes[n1]['radius']) * 1.5, return_distance =True)
        if len(indices[0])<low2*2:
            ...
            distances, indices = kdtree.query([net.graph.nodes[n1]['center']],k=min(high2*2,len(nodelist)),return_distance = True)
        ...
        dist_and_labels = dist_and_labels[:min(high2*2,len(dist_and_labels))]       #On ne garde que les high2*2 premiers pores
        ...
        for (dist,r,j) in dist_and_labels:
            ...
            if n2 != n1:
                ...
                if dist < 0:
                    warn("Overlapping detected ! Pore {}, R = {}  and {}, R = {}, distance = {}".format(n1,net.graph.nodes[n1]['radius'],n2,net.graph.nodes[n2]['radius'],dist))
                if nthroats >= degree:
                    break
        ...
        if net.graph.degree(n1) < minc:
            ...
            indices = kdtree.query([net.graph.nodes[n1]['center']],k=min(high2*2,len(nodelist)),return_distance = False)
            ...
            neighbors = [(net.graph.nodes[nodelist[n]]['radius'],n) for n in indices[0] if nodelist[n] != n1 if net.graph.has_node(nodelist[n])]
            if (len(neighbors) == 0):
                break
...
def add_throats_by_surf(net,nodes=None,ratio=0.75,TRR=1,minc=0,maxc=np.inf,sort_by_radius=False,border_correction=True):
    ...
    for counter,n1 in enumerate(candidates):
        ...
        if len(indices[0])<maxc*2:
            ...
            distances, indices = kdtree.query([net.graph.nodes[n1]['center']],k=min(maxc*2,len(nodelist)),return_distance = True)
        indices = indices[0]
        #On cherche les pores les plus proches (i.e. la distance entre les deux surfaces la plus faible !)
        ...
        for (dist,r,j) in dist_and_labels:
            ...
            if n2 != n1:
                ...
                elif S1 > corrected_ratio(n1) * 4 * np.pi * r1**2 and net.graph.degree(n1) < minc:  #Force a min connectivity even if covered area > max area
...
def prune_throats(pn,lowc,highc,minc=None):
    ...
    nodelist.sort(key=lambda x: x[1], reverse = True)   #On commence par les noeuds de plus haut degré
...
def aggregates_nodes_if_too_big(pn, nodes=None, only_smaller=True, maxradius = None):
    ...
    while iterate:
        ...
        if nodes is None:
            ...
            rdict.sort(key=lambda tup: tup[1],reverse = True)   #we begin by the larger pores
            l,_ = zip(*rdict)
...
def SetBCnodes_2(net,step=100):
    ...
    net.graph.graph['inner_extent'] = extent        #A modifier en prenant la valeur moyenne des distances de chaque face ?
...
def SetBCnodes(net,thickness,mode='center'):
    ...
    for n in net.graph.nodes:
        ...
        if mode =='center':
            net.graph.nodes[n]['category'] = isinside(net.graph.nodes[n]['center'],net.graph.nodes[n]['radius'],extent,thickness)
        elif mode == 'surface':
             net.graph.nodes[n]['category'] = isinside_surface(net.graph.nodes[n]['center'],net.graph.nodes[n]['radius'],extent,thickness)
