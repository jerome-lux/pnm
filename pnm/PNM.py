...
class pore_network():
    ...
    def __init__(self, network = None):
        ...
        if network is None:
            ...
            self.graph.graph['extent'] = None           #extent is the extent defined by pores surfaces
    ...
    def read_ascii(cls,inputfilename,data=True,legacy=False):
        ...
        with open(inputfilename,'r') as netfile:
            ...
            for i in range(int(header[4])):
                ...
                if data:
                    network.add_throat(pi,pj,radius=float(t[2]),length=float(t[3]),sw=float(t[4]),glabel=int(t[5]))
                else:
        ...
        * w_ind: index du pore du côté duquel se trouve le fluide mouillant
        ...
        with open(outfilename,"w") as netfile:
            ...
            for node in self.graph.nodes:
                ...
                netfile.write("{} {} {} {} {} {} {} {} {}\n".format(n['center'][0],n['center'][1],n['center'][2],n['radius'],0.0,0.0,0.0,0,side))

            for n1,n2 in self.graph.edges:
                netfile.write("{} {} {} {} {} {} {}\n".format(n1,n2,self.graph[n1][n2]['radius'],self.graph[n1][n2]['length'],0.0,0,0))
    ...
    def check_overlapping(self,fit_radius=True, merge=True, mindist='auto',update_geometry = False):
        ...
        pores_radii.sort(key=lambda tup: tup[1],reverse = True)   #we begin by the bigger pores
        ...
        while not stop:
            ...
            for i,n1 in enumerate(index):
                ...
                if self.graph.has_node(n1):
                    ...
                    labels = kdtree.query_ball_point(self.graph.nodes[n1]['center'],2.5*self.graph.nodes[n1]['radius'])
                    labels.remove(i)
                    ...
                    d_and_labels = [(d[j],k) for j,k in enumerate(labels)]   #On commence par la distance la plus faible
                            ...
                            if (dist + self.graph.nodes[n2]['radius'] <=  gap) or (dist < gap and dist + self.graph.nodes[n2]['radius'] > gap and not fit_radius):
                                ...
                                if (self.graph.nodes[n1]['radius'] >= self.graph.nodes[n2]['radius']):
                                    ...
                                    if merge:
                                        ...
                                        self.remove_pore(n1)
                                        print("pore",n1,"and",n2,"overlap: deleting",n2)
                                    break   #On termine l'itération car le pore n1 n'existe plus...
    ...
    def compute_porosity(self):
        ...
        if self.geom_complete:
            ...
            self.pores_volume = np.fromiter(pore_volumes.values(),dtype=np.float).sum()

            throat_volumes = nx.get_edge_attributes(self.graph,'volume')
            self.throats_volume = np.fromiter(throat_volumes.values(),dtype=np.float).sum()

            if self.graph.graph.get('extent',None) is None:         #Compute the extent from pores center and radii

                self.compute_extent()

            self.graph.graph['porosity'] = (self.pores_volume + self.throats_volume) / np.prod(self.graph.graph['extent'])
    ...
    def compute_throats_volume(self):
        ...
        if self.geom_complete:
            ...
            throats_radii = np.fromiter(nx.get_edge_attributes(self.graph,'radius').values(),dtype=np.float)
            throats_length = np.fromiter(nx.get_edge_attributes(self.graph,'length').values(),dtype=np.float)
    ...
    def add_pore(self,radius,center,category=set(),**kwargs):
        ...
        self.graph.add_node(n,radius=radius,center=center,volume=4*np.pi*radius**3/3,category=category,**kwargs)
            ...
            for node in self.graph.nodes:
                try:
                    if self.graph.degree(node)>0:
                        self.graph.nodes[node]['radius'] = np.array([self.graph[node][neighbor]['radius'] for neighbor in self.graph[node]]).max()
                except:
    ...
    def write_props(self,filename=None):
        ...
        self.pnmprops['Pore body Porosity'] = self.pores_volume / np.prod(self.graph.graph['extent'])
        self.pnmprops['Throat Porosity'] = self.throats_volume / np.prod(self.graph.graph['extent'])
