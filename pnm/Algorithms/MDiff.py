...
class molecular_diffusion():
    ...
    def compute_boundary_flow(self,bc_nodes):
        ...
        adjm = nx.adj_matrix(self.pn.graph).tolil()
        #Preparing adjacency matrix
        adjm[:,list(bc_nodes)] = 0     #delete coefficients between two boundary nodes [cols]
        adjm[active_nodes] = 0      #delete coefficients between two free nodes [rows]
        rows,cols = adjm.nonzero()
        try:
            qij_1 = np.array([self._flow_coef(i,j)*(self.x[j]-self.bc_dict[i]['value']) for (i,j) in zip(rows,cols) if self.bc_dict[i]['bctype'] == 'Dirichlet']).sum()
        except KeyError:
            warn('bc_nodes must be a list of boundary nodes only')

        try:
            qij_2 = np.array([self.bc_dict[i]['value']*np.pi*self.pn.graph.nodes[i]['radius']**2 for (i,j) in zip(rows,cols) if self.bc_dict[i]['bctype'] == 'Neumann']).sum()
        except KeyError:
            warn('bc_nodes must be a list of boundary nodes only')
    ...
    def setup_coef_matrix(self):
        ...
        adjm = nx.adj_matrix(self.pn.graph).tolil()  #for fast slicing ops
    ...
    def setup_diffusivity_model(self,model='Fick',scale=1,**kwargs):
        ...
        D_Knudsen = kwargs.get('d_knudsen',DEFAULT_DKNUDSEN)        #Knudsen diffusivity without the radius term
        D_Fick = kwargs.get('d_fick',1)                             #Bulk diffusivity is a constant given by the user
    ...
    def setup_throat_radius_model(self,model='throat',alpha = 1):
        ...
        if model == 'throat':
            ...
            def radius(ni,nj):
                ...
                H = math.sqrt(maxr**2 - rij**2) #longeur du pore du centre jusqu'à l'intersection du capillaire
                Reff = maxr*math.sin(math.acos(rij/maxr))   #rayon "moyen" de 0 à H
...
class molecular_diffusion_deprecated():
    ...
    def get_concentration(self,n1):
        ...
        if bctype is None:
            ...
            for n2 in [ n for n in self.pn.graph[n1] if self.bctype_dict.get(n2,None) is None]: #Adjacent inner pores
                return (self.pn.graph[n1][n2]['length']*self.bcvalues_dict[n1] /
                (self.pn.get_throat_section(n1,n2)*self.diffusivity(n1,n2)) + self.x[self.nodes_to_indices[n2]])
    ...
    def add_bc(self,bc_val,nodes,bc_type='Dirichlet'):
        ...
        if bc_type == 'Dirichlet':
            ...
            def get_bc(n1,n2):
                ...
                return bc_val*coef,coef       #returns contribution for the RHS and the matrix diagonal coeff

        elif bc_type == 'Neumann':

            def get_bc(n1,n2):
                return - bc_val*np.pi*self.radius(n1,n2)**2, 0   #returns contribution for the RHS and the matrix diagonal coeff
