...
class molecular_diffusion():
    ...
    def solve(self, solver='cg', x0=None, tol=1e-6, maxiter=None):
        ...
        try:
            self.x, self.iter = solver(
                self.A, self.b, x0=x0, tol=tol, maxiter=maxiter)  # ,M=M
        except:
    ...
    def compute_boundary_flow(self, bc_nodes):
        ...
        try:
            qij_1 = np.array([self._flow_coef(i, j)*(self.x[j]-self.bc_dict[i]['value'])
                              for (i, j) in zip(rows, cols) if self.bc_dict[i]['bctype'] == 'Dirichlet']).sum()
        except KeyError:
            warn('bc_nodes must be a list of boundary nodes only')

        try:
            qij_2 = np.array([self.bc_dict[i]['value']*np.pi*self.pn.graph.nodes[i]['radius'] **
                              2 for (i, j) in zip(rows, cols) if self.bc_dict[i]['bctype'] == 'Neumann']).sum()
        except KeyError:
            warn('bc_nodes must be a list of boundary nodes only')
    ...
    def setup_coef_matrix(self):
        ...
        if len(dirichlet_nodes) > 0:
            ...
            bcij = np.array([self._flow_coef(i, j)*self.bc_dict[i]['value']
        ...
        aii = np.array([self._flow_coef(i, j)
    ...
    def setup_throat_radius_model(self, model='throat', alpha=1):
        ...
        if model == 'throat':
            ...
            def radius(ni, nj):
                ...
                maxr = max(
                    self.pn.graph.nodes[ni]['radius'], self.pn.graph.nodes[nj]['radius'])
    ...
    def add_bc(self, bc_val, nodes, bc_type='Dirichlet'):
        ...
        self.bc_dict.update(
            {i: {'value': bc_val, 'bctype': bc_type} for i in nodes})
...
class molecular_diffusion_deprecated():
    ...
    def solve(self, solver='cg', x0=None, tol=1e-6, maxiter=None, precond=False):
        ...
        if precond:
            ...
            try:
                self.x, self.iter = solver(
                    self.A, self.b, x0=x0, tol=tol, maxiter=maxiter)
            except:
                warn('Solving linear system failed. You should check matrix and RHS')
    ...
    def setup_coef_matrix(self):
        ...
        self.A = sp.coo_matrix((data, (rows, cols)), shape=(
            nnodes, nnodes), dtype=np.float64).tocsr()
