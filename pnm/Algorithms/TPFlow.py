...
class TPFlow():
    ...
    def set_params(self,**kwargs):
        ...
        self.avgsw_stopping_criterion = kwargs.get('avgsw_stopping_criterion',1e-3)
    ...
    def psolve(self,solver='cg',pfield0=None,tol=1e-10,maxiter=None):
        ...
        try:
            self.pfield, self.iter = solver(self.A,self.b,x0=pfield0,tol=tol,maxiter=maxiter) #,M=M
        except:
    ...
    def get_conductance(self,i,j):
        ...
        return (np.pi*self.radius(i,j)**4 /
                ...
                self.pn.graph.nodes[j]['radius']*(swj*self.wf_viscosity + (1-swj)*self.nwf_viscosity)
                ))
                )
    ...
    def setup_simulation(self):
        ...
        #Adjacency matrix
        self.adjm = nx.adj_matrix(self.pn.graph).tolil()        #convert to lil for fast slicing ops

        self.inlet_pores = list(filter(lambda x: self.bc_dict[x]['sw'] > 0, self.bc_dict.keys()))
        self.outlet_pores = list(filter(lambda x: self.bc_dict[x]['sw'] <= 0, self.bc_dict.keys()))

        nx.set_node_attributes(self.pn.graph, dict(zip(self.inlet_pores,'inlet')),'boundary')
        nx.set_node_attributes(self.pn.graph, dict(zip(self.outlet_pores,'outlet')),'boundary')
        ...
        self.p = np.zeros(self.pn.graph.number_of_nodes(),dtype=np.float64) + self.p0
        ...
        self.CClabels = np.zeros(self.pn.graph.number_of_nodes(), dtype = np.int32)
        #status of pores: 0=trapped, 1=connected to the outlet
        self.node_status = np.zeros(self.pn.graph.number_of_nodes(), dtype = np.int8)
    ...
    def update_connected_components(self):
        ...
        unsaturated_pores = np.where(self.swp < 1)       #index of unsaturated pores
    ...
    def label_connected_pores(self,n1,CClabel):
        ...
        while plist:
            ...
            if(self.pn.graph.nodes[i]['boundary'] == 'outlet'):
                status = 1
            nlist = filter(lambda x: self.pn.graph[i][x]['sw'] <= 0 and self.CClabels[x] == 0, self.pn.graph.neighbors(i))
            plist.extend(nlist)
    ...
    def setup_coef_matrix(self):
        ...
        pcij = np.array([-self._get_coef(i,j)*self.get_pc(i,j) for i,j in zip(rows,cols)])  #pc(i,j)=0 if one phase or no valid config.
        PCIJ = sp.coo_matrix((pcij, (rows,cols)),shape = (n,n), dtype=np.float64).tocsr()
        ...
        if (len(neumann_nodes)) > 0:
            ...
            self.b[neumann_nodes] = - np.array(values)  #values est directement la densité de flux imposée*section du pore externe
        ...
        if len(dirichlet_nodes) > 0:
            ...
            adjm1[active_nodes] = 0  #On met à zéro les lignes de la matrice d'adjacence qui corresondent aux noeuds libres
            adjm1[:,dirichlet_nodes] = 0    #On met à zéro les termes qui correspondent au flux entre deux noeuds aux limites sur les colonnes
            brows,bcols = adjm1.nonzero()
            bcij = np.array([self._get_coef(i,j)*self.bc_dict[i]['value'] for (i,j) in zip(brows,bcols)])
        ...
        aii = np.array([self._get_coef(i,j) for (i,j) in zip(diag,all_cols)])
    ...
    def add_bc(self,bc_val,sw,nodes,bc_type='Dirichlet'):
        ...
        self.bc_dict.update({i:{'value':bc_val,'bctype':bc_type,'phase':sw} for i in nodes})
