...
def stats(pn,bins=15,mode='vol',xscale='linear',elements='all',unit="nm",filename = None, show=True):
    ...
    p_radii = np.fromiter(nx.get_node_attributes(pn.graph,'radius').values(),dtype=np.float)
    t_radii = np.fromiter(nx.get_edge_attributes(pn.graph,'radius').values(),dtype=np.float)
    p_vol = np.fromiter(nx.get_node_attributes(pn.graph,'volume').values(),dtype=np.float)
    t_vol = np.fromiter(nx.get_edge_attributes(pn.graph,'volume').values(),dtype=np.float)
