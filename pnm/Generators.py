...
def isinside(coords,radius,extent,epsilon):
    ...
    warn("No Category affected to pore center{} with radius {} (min={},max={}".format(coords,radius,minr,maxr))
    return None
...
def deprec_random_packing( extent,
    ...
    while not length_OK:
        ...
        if extent.min()*length < lim:
            print("Computed min side length {} is smaller than the sum of the 2 largest pores diameter {}".format(extent.min()*length,max_radius))
            ...
            if distances.min() > r + gap:
                ...
                if category is not None:
                    ...
                    # if i*100 % len(radii) == 0:
                        # print ("#")
                    print ("Pore {} with radius {:.2e} added after {} trials. Current porosity: {:.2f} % - unaloccated pores: {}   ".format(len(occupied_centers),r,j+1,100*void_volume / total_volume,len(unallocated_radii)),end="\r")
                    # del(radii[j])
                    break
            # else:
                # todelete.append(j)
    ...
    print("Unallocated volume: {:.2e}, corresponding void_fraction: {:.2f} %".format(unallocated_volume,100*unallocated_volume/total_volume))
...
@jit(nopython=True,fastmath=True)   #'float64(float64[:],float64[:,:],float64[:])',
...
@jit('float64(float64[:],float64[:])',nopython=True,fastmath=True,parallel=True,nogil=True) #'float64(float64[:],float64[:])',
...
#The Fastest !
@jit('float64(float64[:],float64[:,:],float64[:])',nopython=True,fastmath=True,parallel=True,nogil=True) #'float64(float64[:],float64[:,:],float64[:])',
...
@jit(nopython=True,fastmath=True,parallel=True,nogil=True) #'float64(float64[:],float64[:,:],float64[:])',
...
def random_packing_opt(extent,
    ...
    if fit_box:
        ...
        counter = 0
        while abs(temp_porosity - target_porosity) > 0.0005 and counter<5000:  #Si l'écart est supérieur à 0.05%
            counter += 1
        ...
        print("{} pores will be generated to fit the target porosity of {}% in the given box".format(nbnodes,100*temp_porosity))
        max_radius = max(radii)
    ...
    for i in range(len(radii)):
        ...
        for j in range(nb_trials):
            ...
            if distance > r + gap :
                ...
                void_volume += np.pi*(4/3)*r**3
                pore_added = True
                print (" {:14} {:16.2e} {:16} {:16.2f} % {:19}   ".format(len(occupied_centers),r,trials,100*void_volume / total_volume,len(unallocated_radii)),end="\r")
                # del(radii[j])
                break
            # else:
                # todelete.append(j)
        ...
        if not pore_added:
            ...
            if restart and unallocated_volume > 0.05*target_porosity*total_volume : #Si le volume non alloué dépasse 5% du volume poreux cible
                print("\n!!!--- Restarting ---!!!\n")
    ...
    for i in range(len(radii)):
        ...
        for j in range(nb_trials):
            ...
            if len(occupied_centers)> 0:
                ...
                elif len(labels) > mp_nbpoints: #Multiprocessing slows down the computation...
                    with multiprocessing.Pool(processes=4) as pool:
                        distances = 0.998 * pool.map(partial(eucl_dist,np.array([coords])), np.array(occupied_centers)[labels]) - np.array(occupied_radii)[labels]
                else:
                    distances = 0.998 * eucl_dist([coords],np.array(occupied_centers)[labels]) - np.array(occupied_radii)[labels]
